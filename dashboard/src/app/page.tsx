...
import {
    LayoutDashboard, Bot, Key, BarChart3, Settings, LogOut,
    Plus, Copy, Trash2, Check, AlertCircle,
    Zap, Shield, Globe, Server, Activity,
    Wallet, Hash, Loader2, User as UserIcon,
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <CardFooter className="flex flex-col gap-3">
                            ...
                            <p className="text-sm text-muted-foreground">
                                ...
                                <button type="button" className="font-medium underline underline-offset-4 hover:text-foreground transition-colors" onClick={() => { setIsLogin(!isLogin); setError("") }}>
            ...
            <aside className="hidden md:flex w-60 flex-col border-r bg-card/50 fixed inset-y-0 left-0 z-40">
                ...
                <div className="flex items-center gap-2.5 px-5 py-5">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-muted-foreground leading-none mt-0.5">AI Infrastructure</p>
                    </div>
                </div>
                        ...
                        <DropdownMenuTrigger asChild>
                            <button className="flex items-center gap-2.5 w-full px-2.5 py-2 rounded-md hover:bg-accent transition-colors text-left">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium truncate leading-none">{user.name || user.email.split("@")[0]}</p>
                                    <p className="text-[11px] text-muted-foreground truncate leading-none mt-0.5">{user.email}</p>
                                </div>
...
function OverviewPage() {
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/v1/models`).then(r => r.json()).then(d => setModelCount(d.data?.length || 0)).catch(() => { })
        apiRequest("GET", "/billing/balance").then(setBalance).catch(() => { })
    }, [apiRequest])
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    {health ? `Running in ${health.mode} mode • v${health.version}` : "Connecting to backend…"}
                </p>
            </div>

            {/* Stat cards */}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard icon={Activity} label="Status" value={health ? "Online" : "Offline"} sub={health ? `${health.mode} deployment` : "Cannot reach backend"}>
                    {health && <span className="inline-block h-2 w-2 rounded-full bg-green-500 status-pulse mr-1.5" />}
                ...
                <StatCard icon={Wallet} label="Balance" value={balance?.unlimited ? "Unlimited" : `$${balance?.balance?.toFixed(2) || "0.00"}`} sub={balance?.unlimited ? "Local mode" : "USD"} />
                <StatCard icon={Globe} label="Mode" value={health?.mode ? health.mode.charAt(0).toUpperCase() + health.mode.slice(1) : "—"} sub="Deployment" />
            </div>
...
function StatCard({ icon: Icon, label, value, sub, children }: {
    icon: any; label: string; value: string; sub: string; children?: React.ReactNode
}) {
    return (
        <Card>
            ...
            <CardContent>
                <div className="text-2xl font-semibold flex items-center">{children}{value}</div>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">All registered models across providers</p>
                </div>
                ...
                <Table>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow><TableCell colSpan={4} className="text-center py-12 text-muted-foreground">No models registered. Start the backend to seed defaults.</TableCell></TableRow>
                        ) : models.map(m => (
                            <TableRow key={m.id}>
                                <TableCell className="font-mono text-sm">{m.id}</TableCell>
                                <TableCell><Badge variant="outline">{m.owned_by}</Badge></TableCell>
                                <TableCell><Badge variant="secondary" className="text-green-600 dark:text-green-400">Active</Badge></TableCell>
                                <TableCell>
                                    <Tooltip><TooltipTrigger asChild>
                                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { navigator.clipboard.writeText(m.id) }}>
                                            <Copy className="h-3.5 w-3.5" />
                                        </Button>
                                    </TooltipTrigger><TooltipContent>Copy model ID</TooltipContent></Tooltip>
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
...
function ApiKeysPage() {
    ...
    const loadKeys = async () => {
        try { const data: any = await apiRequest("GET", "/auth/api-keys"); setKeys(data) } catch { }
        setLoading(false)
    }
    ...
    const deleteKey = async (id: string) => {
        ...
        try { await apiRequest("DELETE", `/auth/api-keys/${id}`); loadKeys() } catch { }
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Create and manage authentication keys</p>
                </div>

                <Dialog open={showCreate} onOpenChange={(o) => { setShowCreate(o); if (!o) setNewKey(null) }}>
                    <DialogTrigger asChild>
                        <Button size="sm"><Plus className="mr-1.5 h-4 w-4" />Create Key</Button>
                    </DialogTrigger>
                            ...
                            <>
                                <DialogHeader>
                                    <DialogTitle className="flex items-center gap-2">
                                        <Sparkles className="h-4 w-4" />Key Created
                                    </DialogTitle>
                                    <DialogDescription>Copy your key now — you won&apos;t see it again.</DialogDescription>
                                </DialogHeader>
                                <div className="space-y-3 py-2">
                                    <div className="flex items-center gap-2">
                                        <code className="flex-1 rounded-md border bg-muted/50 p-3 text-sm font-mono break-all select-all">{newKey}</code>
                                    ...
                                    <div className="flex items-start gap-2 text-sm text-muted-foreground bg-muted/50 rounded-md p-3 border">
                                        ...
                                        <span>Store this key in a secure location. It will not be displayed again after you close this dialog.</span>
                                    </div>
                                </div>
                                <DialogFooter>
                                    <Button className="w-full sm:w-auto" onClick={() => { setShowCreate(false); setNewKey(null) }}>Done</Button>
                                </DialogFooter>
                            </>
                        ) : (
                            <>
                                <DialogHeader>
                                    ...
                                    <DialogDescription>Generate a new key for PIEE API authentication.</DialogDescription>
                                </DialogHeader>
                                <div className="space-y-3 py-2">
                                    <div className="space-y-2">
                                        <Label htmlFor="keyName">Key Name</Label>
                                        <Input id="keyName" placeholder="e.g. Production, Development, CI/CD" value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createKey()} />
                        ...
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={6} className="text-center py-16">
                                    ...
                                    <p className="text-xs text-muted-foreground mt-1">Create your first key to start using the API</p>
                                </TableCell>
                            </TableRow>
                        ) : keys.map(k => (
                            <TableRow key={k.id}>
                                <TableCell className="font-medium">{k.name}</TableCell>
                                <TableCell><code className="text-xs text-muted-foreground">{k.key_prefix}</code></TableCell>
                                <TableCell>
                                ...
                                <TableCell>
                                    <Tooltip><TooltipTrigger asChild>
                                        <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => deleteKey(k.id)}>
...
function UsagePage() {
    ...
    useEffect(() => {
        apiRequest("GET", "/audit/usage?limit=50").then(setStats).catch(() => { }).finally(() => setLoading(false))
    }, [apiRequest])
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Monitor API requests, tokens, and costs</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-3">
                <StatCard icon={Hash} label="Total Requests" value={stats?.total_requests?.toLocaleString() || "0"} sub="All time" />
                <StatCard icon={Zap} label="Total Tokens" value={stats?.total_tokens?.toLocaleString() || "0"} sub="Input + output" />
                <StatCard icon={Wallet} label="Total Cost" value={`$${stats?.total_cost?.toFixed(4) || "0.0000"}`} sub="Based on model pricing" />
            </div>
                ...
                <CardContent>
                    {loading ? (
                            ...
                            <TableBody>
                                {stats.breakdown.slice(0, 20).map((r: any) => (
                                    <TableRow key={r.id}>
                                        ...
                                        <TableCell><Badge variant={r.status_code === 200 ? "secondary" : "destructive"}>{r.status_code}</Badge></TableCell>
                        ...
                        <div className="text-center py-16">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">Make API requests to see analytics here</p>
                        </div>
                    )}
                </CardContent>
            </Card>
...
function SettingsPage({ user }: { user: { id: string; email: string; name: string | null; role: string } }) {
    const [health, setHealth] = useState<any>(null)
            ...
            <div className="grid gap-4 md:grid-cols-2">
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        ...
                        <Separator />
                        <Row label="Role"><Badge variant="outline">{user.role}</Badge></Row>
                        <Separator />
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        <Row label="Mode"><Badge variant="secondary">{health?.mode?.toUpperCase() || "—"}</Badge></Row>
                        <Separator />
...
function Row({ label, value, mono, muted, children }: {
    label: string; value?: string; mono?: boolean; muted?: boolean; children?: React.ReactNode
}) {
    return (
        <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">{label}</span>
            {children || (
                <span className={`${mono ? "font-mono text-xs" : ""} ${muted ? "text-muted-foreground" : ""} truncate max-w-[60%] text-right`}>
                    {value}
                </span>
            )}
        </div>
